...
%
\\[5mm]What we call regret at iteration $t$ is $\theta^{\top}(x_{a_t^*} - x_{a_t})$ where $a_t$ was the arm pulled and $a_t^*$ was the arm maximizing the expected reward (${a_t^* = \mathrm{argmax}_{a \in \mathcal{A}_t} (\theta^{\top} x_a)}$).
\\Since the noise is centered and assumed to be the same for all arms, our regret is just the expectation of the difference between the maximum reward and the reward received.
